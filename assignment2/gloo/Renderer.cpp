...
void Renderer::RecursiveRetrieve(const SceneNode& node,
  ...
  const glm::mat4& model_matrix)const{
    //a recursive depth-first search that traverses the scene tree while maintaining the current local-to-world matrix.
    //It starts from the root node and recursively processes each child node, updating the model matrix for each child.
    int child_count = node.GetChildrenCount();
    //std::cerr << "RecursiveRetrieve: " << info.size() << std::endl;
    //go through all children
    for (int i = 0; i < child_count; i++) {
      const SceneNode& child = node.GetChild(i);
      //get active children
      if (child.IsActive()) {
        glm::mat4 child_model_matrix = model_matrix * child.GetTransform().GetLocalToParentMatrix();
        if(auto rc = child.GetComponentPtr<RenderingComponent>()) {
          info.emplace_back(rc, child_model_matrix);
        }
